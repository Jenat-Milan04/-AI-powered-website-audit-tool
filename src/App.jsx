...
function ImpactBadge({ impact }) {
  const map = { high: styles.impHigh, medium: styles.impMed, low: styles.impLow }
...
export default function App() {
  ...
  useEffect(() => {
    fetch('https://ai-powered-website-audit-tool.onrender.com/health')
    ...
    try {
      ...
      if (result && result.summary && result.score !== undefined) {
        ...
        setAiResult({
          ...
          score: 50,
          insights: [{ category: 'Info', status: 'warning', headline: 'Partial Analysis', text: 'The AI response was incomplete. Please try again.' }],
          recommendations: [{ priority: 1, impact: 'medium', title: 'Try Again', text: 'Please run the audit again for complete results.' }],
        })
        setStep('done')
      }
      setPromptLog(log)
            ...
            {promptLog && (
              <>
                ...
                <div className={styles.sectionLabel}>
                  ...
                  Prompt log
                  <button className={styles.toggleBtn} onClick={() => setShowLog(v => !v)}>
                    {showLog ? 'Hide ▲' : 'Show ▼'}
                  </button>
                </div>
